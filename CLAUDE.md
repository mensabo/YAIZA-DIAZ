...
## Auditoría Lighthouse (23/07/2026)

- Home en 77 Performance / 100 en el resto; techo asumido (ver hero/cookies arriba). Páginas con grids de Firestore (`eventos`, `entrevistas`, programas TV en `comunicacion`) bastante peor: hasta 68-72 Performance porque el LCP real es una imagen que depende de una cadena serie completa (HTML → `script.js` → SDK Firebase → query a Firestore → imagen de Firebase Storage) — Lighthouse penaliza esa cadena fuerte bajo throttling simulado aunque en red real no se note tanto.
- **Bug real encontrado y corregido**: las funciones `loadAndRenderEventsGridPage`, `loadAndRenderInterviews` y `loadAndRenderTVPrograms` en `src/script.js` ponían `loading="lazy"` a **todas** las imágenes del grid, incluida la primera (que suele ser el LCP). Ahora solo la primera tarjeta de cada lista carga sin `lazy` (índice 0), el resto sigue lazy. Si se añade una lista similar nueva, replicar el patrón: `index === 0 ? '' : ' loading="lazy"'`.
- Se añadió `preconnect` a `firestore.googleapis.com` y `firebasestorage.googleapis.com` en las 17 páginas públicas (junto al de `cdnjs.cloudflare.com` ya existente, dentro del presupuesto de 3-4 preconnects) para acortar esa cadena — ayuda al handshake pero no elimina la dependencia serie de fondo.
- `tests/audit.test.js` tenía dos checks obsoletos que llevaban tiempo fallando sin ser bugs reales: (1) esperaba `preconnect` a `fonts.googleapis.com` aunque las fuentes están auto-alojadas desde hace tiempo — cambiado a comprobar que **no** aparece esa URL; (2) el regex del CTA "Contacto" del menú no contemplaba `aria-current="page"` (que sí lleva `contacto.html` al marcarse a sí misma como página actual) — regex corregido. Antes de asumir que un check que lleva tiempo en FAIL es un bug pendiente, comprobar primero si el sitio cambió de diseño/arquitectura y es el test el que quedó desactualizado.
- El usuario no trabaja en local: no hay ninguna carpeta suya que se sincronice sola con los pushes a GitHub/IONOS. Si pregunta si puede borrar una copia local antigua, confirmar que sí sin problema (el repo remoto es la fuente de verdad, todo push termina ahí).

## Sesión 24/07/2026 — Protección frente a gasto excesivo, App Check, auditoría UX/SEO/CRO

- **Cloud Function `imgCache`** (`functions/index.js`) desplegada y activa: cachea las imágenes de Storage detrás de la CDN de Firebase Hosting (`yaiza-diaz.web.app/img-cache?u=<url>`), en vez de servirlas directo desde `firebasestorage.googleapis.com`. Motivo: cada `<img>` es una petición HTTP normal sin SDK, no puede llevar token de App Check, y cada visitante distinto contaba contra la cuota gratuita de Storage (100GB/mes) — con la CDN compartida delante, miles de peticiones a la misma imagen solo golpean Storage una vez. Activado con `cachedImg()` en `src/script.js` (interruptor `IMG_CACHE_ACTIVO`, patrón reutilizable si hay que desactivarlo de emergencia). Añadido `preconnect` a `yaiza-diaz.web.app` en las 17 páginas públicas.
  - **Susto real durante el despliegue**: un primer commit con `cachedImg()` sin interruptor de seguridad estuvo a punto de desplegarse a IONOS antes de que la Cloud Function existiera de verdad — habría roto todas las imágenes del sitio. Detectado y corregido a tiempo (interruptor `false` por defecto, activar solo tras confirmar que el endpoint responde). **Lección: cualquier cambio que dependa de un recurso externo aún no desplegado (Cloud Function, DNS, etc.) necesita un interruptor `false` por defecto desde el primer commit, nunca añadirlo "después".**
- **App Check en Storage: investigado a fondo, no se puede activar sin romper la web pública.** Las imágenes se sirven con `<img src>` directo (o ahora vía `imgCache`), una petición HTTP normal que nunca puede llevar el token de App Check — forzar "Aplicada" en Storage bloquearía las imágenes de todo el sitio, no solo ataques. Mitigación real implementada: la caché de arriba, no App Check.
- **Alerta de presupuesto de Firebase**: ya existía de antes en la cuenta de facturación "Mi cuenta de facturación 1" (que también cubre este proyecto) — confirmado, no hacía falta crear una nueva.
- **Auditoría UX/SEO/CRO completa (informe en artifact, no en el repo)**: hallazgos aplicados hoy — banner de cookies en móvil tapaba el hero por completo. Causa real, dos bugs de CSS distintos, no solo el texto largo:
  1. `.cookie-banner-settings` tenía el atributo HTML `hidden`, pero la regla `display:flex` de la propia hoja de estilos tiene la MISMA especificidad (selector de atributo vs. de clase) — ganaba la del sitio por orden de cascada, así que el panel de casillas se veía SIEMPRE. Arreglado con `.cookie-banner-settings[hidden]{display:none}` (sube la especificidad).
  2. `.cookie-banner-content{flex:1 1 420px}` está pensado para el ANCHO en escritorio (el banner va en fila ahí) — en móvil el contenedor cambia a `flex-direction:column`, y ese mismo `420px` se convierte en una ALTURA mínima, no un ancho. Arreglado con `flex-basis:auto` dentro del media query móvil.
  - Resultado verificado con Playwright (viewport 390×844): 493px (58% de la pantalla) → 117px (14%). **Lección: al depurar un elemento que "sigue grande" pese a arreglos aparentes, medir con `getBoundingClientRect()` en cada hijo, no asumir que el primer fix visible fue suficiente — puede haber más de una causa apilada.**
  - Pendientes de la auditoría (no implementados hoy, quedan en el roadmap): H1 de la home es solo "YAIZA DÍAZ" sin keyword; falta prueba social (logos de marcas) visible sin scroll; menú de 12 ítems sin agrupar; sin CTA de bajo compromiso además de "Contacto".
